...
def imgid(update , context):
    msg = update.effective_message
    if msg.reply_to_message and msg.reply_to_message.photo:
        update.effective_message.reply_text(
            f"IMG ID:\n<code>{msg.reply_to_message.photo[0].file_id}</code>",
            parse_mode=ParseMode.HTML)
    else:
        update.effective_message.reply_text(
            "Please reply to img to get its ID.")
...
IMGID_HANDLER = DisableAbleCommandHandler("imgid", imgid)
...
dispatcher.add_handler(IMGID_HANDLER)
...
__handlers__ = [
    ...
    SET_ABOUT_HANDLER, GET_ABOUT_HANDLER, CHECKSTATS_HANDLER , IMGID_HANDLER
]
